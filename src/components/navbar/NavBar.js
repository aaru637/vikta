        ...
        <nav className={`flex justify-between min-w-full items-center px-5 py-5 fixed top-0 z-20 text-white ${scrolled ? "bg-black" : "bg-transparent"
            ...
            <ul className='list-none hidden sm:flex flex-row w-[83%] justify-end gap-8'>
                {navbarTitles.map((nav) => (
                    <li key={nav.id} className={`nav-element px-2 py-1 hover:bg-pink-600 hover:rounded-lg hover:text-white font-bold ${active === nav.value ? "text-white bg-pink-600 rounded-lg" : "text-white"}`} >
                        <a href={`#${nav.id}`} onClick={() => setActive(nav.value)} >{nav.value}</a></li>
                ))}
            </ul>
            <div>
